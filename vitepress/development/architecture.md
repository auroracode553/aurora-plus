...
实时示例位于 `.vitepress/theme/examples/`。组件页同时把同一个 `.vue` 文件作为组件和 `?raw` 源码导入：组件负责预览，原始文本负责展开与复制，因此示例数据、事件逻辑和展示源码不会出现两份实现或内容漂移。`DemoBlock.vue` 只负责预览容器、源码折叠和复制交互，不持有业务示例数据。
