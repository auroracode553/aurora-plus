...
| `ariaLabel` | 加载状态的无障碍文案 | `string` | `加载中` |
...
容器使用 `aria-busy` 表达加载或延迟隐藏状态，并提供单一的礼貌状态播报。占位块从无障碍树中隐藏，模板容器使用 `inert` 阻止占位内容获得焦点；模板仅放置展示内容。业务内容在就绪后挂载，保留其自身键盘和焦点行为。

容器背景透明，占位颜色跟随全局语义主题。启用减少动态、减少透明度、高对比度或强制色彩偏好时关闭动画，并按对应偏好提供纯色或轮廓回退。
